...
import { jwks } from '../src/handler';

describe('GET /.well-known/jwks.json', () => {
  it('serves the RS256 public key with the configured kid', async () => {
    const result = (await jwks()) as { statusCode: number; body: string };
  ...
  it('never exposes private key material', async () => {
    const result = (await jwks()) as { body: string };
...
describe('GET /.well-known/openid-configuration', () => {
  it('serves a discovery document pointing at the JWKS', async () => {
    const { openidConfiguration } = await import('../src/handler');
    const event = {
      requestContext: { domainName: 'abc123.execute-api.eu-central-1.amazonaws.com' },
    } as never;
    const result = (await openidConfiguration(event)) as { statusCode: number; body: string };
    expect(result.statusCode).toBe(200);
